...
export = {
    load(rt: CRuntime) {
        ...
        function* _createMapNodeMembersDefault(mapIterType: __node['t']): Gen<__node['members']> {
            const default_key = yield* rt.defaultValue2<__pair>(mapIterType.templateSpec[0], "SELF");
            (default_key.members.first as any).isConst = true;
        ...
        function _createMapIterMembers(nodeType: __node['t'], node: __node | null, last: __node | null): __map_iter['members'] {
            return {
                node: (node !== null) ? {
                    t: {
                        sig: "PTR",
        ...
        function _createMapIterVar(mapIterType: __map_iter['t'], mapNodeType: __node['t'], node: __node | null, last: __node | null): __map_iter {
            return {
                t: mapIterType,
                isConst: false,
                state: "INIT",
        ...
        rt.defineStruct2("{global}", "map_iterator", {
            numTemplateArgs: 1, factory(iterType: __node['t']): __map_iter['members'] {
        ...
        function _createMapMembers(mapType: MapType<ObjectType, ObjectType>): __map['members'] {
        ...
        common.regOps(rt, [
            {
                ...
                default(rt: CRuntime, _templateTypes: [], lmap: __map, rmap: __map): __map {
                    _clear(lmap);
                    const rr: __node | null = variables.asInitDirectPointerPointee(rmap.members.root);
                    if (rr === null) {
                        return lmap;
                    }
                    const nodeType = lmap.members._t_node.t.templateSpec[0];
                    ...
                    variables.directPointerAssign2(rt, lmap.members.root, clone(rr, null));
                    lmap.members._size.value = rmap.members._size.value;
                    return lmap;
                },
        ...
        function _begin(thisVar: __map): __map_iter {
            ...
            return _createMapIterVar(
                thisVar.members._t_iter.t.templateSpec[0],
                thisVar.members._t_node.t.templateSpec[0],
        ...
        function* _find(rt: CRuntime, thisVar: __map, key: Variable): Gen<__map_iter> {
            const iterType = thisVar.members._t_iter.t.templateSpec[0];
            const nodeType = thisVar.members._t_node.t.templateSpec[0];
        ...
        function* _insert(rt: CRuntime, thisVar: __map, value: __pair): Gen<[__map_iter, boolean]> {
            ...
            (value.members.first as any).isConst = true;
            const iterType = thisVar.members._t_iter.t.templateSpec[0];
            const nodeType = thisVar.members._t_node.t.templateSpec[0];
            ...
            let nodeValue: __node = _createMapNodeVar(nodeType, value, true);
            ...
            return [yield* _find(rt, thisVar, value.members.first), true];
        }
        ...
        common.regGlobalFuncs(rt, [{
            ...
            default(rt: CRuntime, _templateTypes: ObjectType[], map: MapVariable<InitArithmeticNumVariable, Variable>): "VOID" {
                ...
                function _map_int_print_tree_inner(node: MapNodeValue<PairVariable<InitArithmeticNumVariable, Variable>>, shift: number): void {
                    ...
                    stdio.write(`${" ".repeat(shift)}${((node.members.is_red.value === 1) ? "R" : "B")}:${node.members.key.members.first.value}\n`);
                    const rhs = variables.asInitDirectPointerPointee(node.members.rhs);
                    if (rhs !== null) {
                        _map_int_print_tree_inner(rhs, shift + 1);
                    }
                }
                const root = variables.asInitDirectPointerPointee(map.members.root);
                if (root !== null) {
        ...
        common.regMemberFuncs(rt, "map", [
            ...
            },
            {
                ...
                *default(rt: CRuntime, _templateTypes: ObjectType[], thisVar: __map, first: Variable, last: Variable): Gen<"VOID"> {
                    ...
                    const valueType = thisVar.members._t_pair.t.templateSpec[0];
