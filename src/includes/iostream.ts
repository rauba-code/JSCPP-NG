...
import { ArithmeticType, ArithmeticVariable, ClassType, ClassVariable, DynamicArrayVariable, IndexPointerVariable, LValueHolder, MaybeLeft, Variable, variables } from "../variables";
...
interface IStreamVariable {
    readonly t: IStreamType,
    readonly v: IStreamValue,
};

interface IStreamValue {
    readonly lvHolder: LValueHolder<IStreamVariable>;
    readonly isConst: boolean;
    members: {
        "buf": IndexPointerVariable<DynamicArrayVariable<ArithmeticVariable>>,
        "fd": ArithmeticVariable,
    },
}
...
interface OStreamVariable {
    readonly t: IStreamType,
    readonly v: IStreamValue,
};

interface OStreamValue {
    readonly lvHolder: LValueHolder<OStreamVariable>;
    readonly isConst: boolean;
    members: {
        "buf": IndexPointerVariable<DynamicArrayVariable<ArithmeticVariable>>,
        "fd": ArithmeticVariable,
...
export = {
    load(rt: CRuntime) {
        ...
        const pchar = variables.indexPointerType(variables.dynamicArrayType(variables.arithmeticType("I8")));
        rt.defineStruct("{global}", "istream", [
            {
                name: "buf",
                variable: variables.indexPointer<ArithmeticVariable>(variables.dynamicArray(pchar.array.object, [], "SELF"), 0, "SELF")
            },
            {
                name: "fd",
                variable: variables.arithmetic("I32", null, "SELF"),
            },
        ]);
